...
def BaseQ_altavg_diffratio(bam_fname, chrom, loc, ref, alt, baseq_char_thres,encoding, max_depth=100000):
	alt_avg = 0.00
	ref_avg = 0.00
	re = ""
			...
			for pileupread in pileupcolumn.pileups:
				if pileupread.alignment.seq[pileupread.qpos] == alt:
					if pileupread.alignment.qual[pileupread.qpos] > baseq_char_thres:
						baseq = baseq_char_ASCII(pileupread.alignment.qual[pileupread.qpos], encoding);
						if baseq in alt_dict.keys():
							alt_dict[baseq] = alt_dict[baseq] + 1
						else:
							alt_dict[baseq] = 1
				elif pileupread.alignment.seq[pileupread.qpos] == ref:
					if pileupread.alignment.qual[pileupread.qpos] > baseq_char_thres:
						baseq = baseq_char_ASCII(pileupread.alignment.qual[pileupread.qpos], encoding);
						if baseq in ref_dict.keys():
							ref_dict[baseq] = ref_dict[baseq] + 1
						else:
							ref_dict[baseq] = 1
	...
	#alt_refdiff_ratio = alt_refdiff/ref_avg
...
def worker(var_file_nm, out_file_nm, pid, nproc, bam_fname, baseq_char_thres, base_qual_encoding):
	...
	for line in var_file:
		if ln % nproc == pid:
			...
			ref = cols[2]
			alt = cols[3]
			re = BaseQ_altavg_diffratio(bam_fname, chrom, loc, ref, alt,baseq_char_thres, base_qual_encoding)
...
def main():
	...
	if len(opt) < 7:
		print("""Usage: ./BaseQ_by_alt_avg_diffratio.py -p # var.bam var.txt baseq_thres base_qual_encoding out.txt
...
avg_alt: average alternative reads BaseQ from both strands
avg_ref: average reference reads BaseQ from both strands
alt_refdiff: alt_avg - ref_avg
""")
		sys.exit(1)
	nproc = int(opt[1])
	...
	out_file_fh.close()
	var_file.close()
	sys.stdout.write("start multiprocessing\n")
	processes = [mp.Process(target=worker, args=(var_file_nm, out_file_nm, x, nproc, bam_fname, baseq_char_thres, base_qual_encoding)) for x in range(nproc)]
	# Run processes
	for p in processes:
		p.start()
